...
extern "C++" {
    template <size_t N>
    char _RTL_CONSTANT_STRING_type_check(const char (&str)[N]);
    template <size_t N>
    char _RTL_CONSTANT_STRING_type_check(const char (&str)[N]);
    template <size_t N>
    char _RTL_CONSTANT_STRING_type_check(const WCHAR (&str)[N]);
...
using POBJECT_ATTRIBUTES32 = OBJECT_ATTRIBUTES32 *;

using PCOBJECT_ATTRIBUTES32 = const OBJECT_ATTRIBUTES32 *;
...
enum NT_PRODUCT_TYPE {
    NT_ProductWinNt = 1,
    NT_ProductLanManNt = 2,
    NT_ProductServer = 3,
};
using PNT_PRODUCT_TYPE = NT_PRODUCT_TYPE *;
...
using PCLIENT_ID32 = CLIENT_ID32 *;
...
using PCLIENT_ID64 = CLIENT_ID64 *;
...
#define ASSERTMSG(msg, exp) \
    ((!(exp)) ? (RtlAssert((PVOID) #exp, (PVOID)__FILE__, __LINE__, (PSTR)msg), FALSE) : TRUE)
#endif
...
#define NT_ASSERT_ACTION(_exp)                                      \
    ((!(_exp)) ? (__annotation(L"Debug", L"AssertFail", L## #_exp), \
...
#define NT_ASSERTMSG_ACTION(_msg, _exp)                           \
    ((!(_exp)) ? (__annotation(L"Debug", L"AssertFail", L##_msg), \
                  DbgRaiseAssertionFailure(), FALSE)              \
               : TRUE)
#endif
...
#define NT_ASSERTMSGW_ACTION(_msg, _exp)                       \
    ((!(_exp)) ? (__annotation(L"Debug", L"AssertFail", _msg), \
                  DbgRaiseAssertionFailure(), FALSE)           \
               : TRUE)
#endif
